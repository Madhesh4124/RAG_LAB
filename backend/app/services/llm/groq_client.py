...
@lru_cache(maxsize=8)
def _build_llm(model_name: str, temperature: float, api_key: str) -> ChatGroq:
...
class GroqClient:
    """Groq Client using LangChain's ChatGroq."""
    ...
    def __init__(self, model: str = None, temperature: float = 0.2, system_prompt: str = None):
        ...
        logger.info(
            "Initializing Groq LLM client model=%s temperature=%s api_key_present=%s key_prefix=%s",
            self.model_name,
            self.temperature,
            bool(api_key),
    ...
    def generate(self, query: str, chunks: List[Chunk]) -> str:
        ...
        if not self.llm:
            raise RuntimeError(
                "LLM client not initialized. GROQ_API_KEY is missing."
            )
